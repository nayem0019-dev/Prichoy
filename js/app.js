...
// ── AUTH — Phase 5: real customer API (replaces localStorage-only demo) ──────
async function doLogin(e){
  ...
  try {
    const res = await customerApi.login(phone, pass);
    const cust = res.data?.customer || customerAuth.getCustomer();
    if (cust) {
      state.user = { name: cust.name, email: cust.email, phone: cust.phone, lastActive: Date.now() };
      saveState();
    }
    refreshCartUI(); refreshWishUI(); refreshAuthUI(); closeAuth();
    toast(`Welcome back, ${cust?.name?.split(' ')[0] || 'there'}! 🌸`, "ok");
  } catch(err) {
    toast(err.message || "Invalid phone or password", "err");
...
async function doRegister(e){
  ...
  const email=document.getElementById("r-email").value.trim()||undefined;
  if(pass.length<6){toast("Password must be 6+ characters","err");btn.disabled=false;btn.textContent='Create Account';return;}
  try {
    await customerApi.register(name, phone, pass, email);
    state.user = { name, phone, email, lastActive: Date.now() };
    saveState(); refreshCartUI(); refreshWishUI(); refreshAuthUI(); closeAuth();
    toast(`Welcome to Prichoy, ${name.split(' ')[0]}! 🌸`, "ok");
  } catch(err) {
