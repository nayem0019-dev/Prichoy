...
export function authorize(...roles: Role[]) {
  return (req: AuthRequest, res: Response, next: NextFunction): void => {
    ...
    // Phase 6.5 §3 — OWNER always passes any authorization check.
    // The OWNER role is the only role that can never be removed or
    // restricted. Full permission management (which roles can do what)
    // is a Phase 7 task; here we just ensure OWNER is never locked out.
...
// Phase 6.5 §3 — Guard that prevents deletion or demotion of OWNER accounts.
// Import and use this in the admin user management controller (Phase 7)
// before any user.update / user.delete that targets a user by ID.
export function protectOwnerAccount(targetRole: string, action: 'delete' | 'demote'): void {
  if (targetRole === 'OWNER') {
    throw Object.assign(new Error(`Owner account cannot be ${action}d`), { statusCode: 403 });
  }
}
