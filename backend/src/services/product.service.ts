...
import { ProductStatus, ProductLabel } from '../types';
...
export class ProductService {
  ...
  async setLabels(productId: string, labels: string[], adminId?: string) {
    await this.getById(productId);
    const VALID_LABELS = ['NEW','BEST_SELLER','TRENDING','SALE','LIMITED','PREMIUM','FEATURED'];
    const validLabels = labels.filter((l) => VALID_LABELS.includes(l)) as ProductLabel[];
