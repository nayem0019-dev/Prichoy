  ...
  async getSalesSummary(startDate: Date, endDate: Date) {
    const orders = await prisma.order.findMany({
      where: {
        ...
        status: { notIn: ['CANCELLED', 'RETURNED'] }, isTest: false,
      },
      include: { items: { include: { product: { select: { costPrice: true } }, variant: { select: { price: true } } } } },
    });
    ...
    // Phase 4 §24 fix — this previously multiplied `unitPrice` (the price
    // the customer was charged) by quantity, which isn't the cost of goods
    // at all; it made costOfGoods ≈ totalRevenue and netProfit collapse to
    // roughly `-totalExpenses` regardless of actual margins. Cost of goods
    // must come from what the product actually cost to acquire
    // (Product.costPrice), not what it sold for.
    const costOfGoods    = orders.reduce((s, o) =>
      s + o.items.reduce((si, i) => si + Number(i.product?.costPrice ?? 0) * i.quantity, 0), 0
    );
    ...
    return {
      ...
      averageOrder, totalExpenses, costOfGoods, grossProfit, netProfit,
