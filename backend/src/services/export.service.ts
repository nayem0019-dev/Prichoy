...
export class ExportService {
  ...
  // ── Phase 4 §19 — Financial report data exports ─────────────────

  async getSalesReportData(filter: { startDate?: string; endDate?: string; preset?: string }): Promise<Record<string, unknown>[]> {
    const { resolveDateRange } = await import('../utils/dateRange');
    const { start, end } = resolveDateRange(filter);
    const orders = await prisma.order.findMany({
      where: { createdAt: { gte: start, lte: end }, status: { notIn: ['CANCELLED', 'RETURNED'] } },
      include: {
    ...
    return orders.map((o) => ({
      ...
      'Discount (৳)': Number(o.discountAmount),
      'Courier (৳)': Number(o.courierCost ?? 0),
      'COGS (৳)': o.items.reduce((s, i) => s + Number(i.product?.costPrice ?? 0) * i.quantity, 0),
      'Gross Profit (৳)': Number(o.grandTotal) - o.items.reduce((s, i) => s + Number(i.product?.costPrice ?? 0) * i.quantity, 0),
    }));
  }

  async getExpensesData(filter: { startDate?: string; endDate?: string; category?: string }): Promise<Record<string, unknown>[]> {
    const where: Record<string, unknown> = {};
    if (filter.startDate || filter.endDate) {
      where.date = {};
    ...
    const expenses = await prisma.expense.findMany({ where: where as never, orderBy: { date: 'desc' } });
  ...
  async getReturnsData(): Promise<Record<string, unknown>[]> {
    ...
    return returns.map((r) => ({
      ...
      'Recovery Action': r.recoveryAction ?? '',
      'Outbound Charge (৳)': Number(r.outboundCharge ?? 0),
      'Outbound Paid By': r.outboundPaidBy ?? '',
      'Return Charge (৳)': Number(r.returnCharge ?? 0),
      'Return Paid By': r.returnPaidBy ?? '',
      'Packaging Cost (৳)': Number(r.packagingCost ?? 0),
      'Resale Value (৳)': Number(r.resaleValue ?? 0),
      'Total Loss (৳)': Number(r.totalLoss ?? 0),
      'Date': r.createdAt.toISOString().slice(0, 10),
    }));
  }

  async getExchangesData(): Promise<Record<string, unknown>[]> {
    ...
    return exchanges.map((e) => ({
      'Order Number': e.order.orderNumber,
      'Customer': e.order.customer?.name ?? '',
      'Phone': e.order.customer?.phone ?? '',
      'Requested Size': e.requestedSize ?? '',
      'Requested Color': e.requestedColor ?? '',
      'Replacement Variant': e.requestedVariant ? `${e.requestedVariant.name}: ${e.requestedVariant.value}` : '',
      'Status': e.status,
      'Courier Charge (৳)': Number(e.courierCharge ?? 0),
      'Courier Paid By': e.courierPaidBy ?? '',
