import { EmailTypeEnum as EmailType } from '../types';
import { OrderStatus, PaymentMethod, ContactMethod, ContactOutcome } from '../types';
...
export class OrderService {
  ...
  async create(data: CreateOrderData, adminId?: string) {
    ...
    // Phase 6.5 §1 — check sandbox mode. If active, the order is created
    // with isTest=true. Stock is still reduced (so the sandbox tests the
    // full flow) but analytics queries always filter isTest=false.
    let isTest = false;
    try {
      const { sandboxService } = await import('./sandbox.service');
      isTest = await sandboxService.isActive();
    } catch { /* sandbox service failure must never block order creation */ }
  ...
  async updateStatus(
    ...
    // Phase 6 §21 — automated notifications on key status changes
    try {
      const { notificationService } = await import('./notification.service');
      await notificationService.notifyOrderStatus(order, newStatus);
    } catch { /* notification failure must never break the order flow */ }

    // Phase 6 §6 — issue welcome-back coupon on first delivery
    if (newStatus === 'DELIVERED') {
      try {
        const { welcomeBackOfferService } = await import('./welcomeBack.service');
        welcomeBackOfferService.maybeIssue(orderId);
      } catch { /* non-blocking */ }
    }
