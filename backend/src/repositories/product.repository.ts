...
export class ProductRepository {
  async findAll(filter: ProductFilterQuery & { includeDeleted?: boolean }) {
    ...
    if (filter.gender)              where.gender     = filter.gender as any;
    ...
    if (filter.label) {
      where.labels = { some: { label: filter.label as any } };
    }
    ...
    if (filter.createdFrom || filter.createdTo) {
      ...
      if (filter.createdTo)   (where.createdAt as any).lte = new Date(filter.createdTo + 'T23:59:59');
    }
    if (filter.updatedFrom || filter.updatedTo) {
      where.updatedAt = {};
      if (filter.updatedFrom) (where.updatedAt as any).gte = new Date(filter.updatedFrom);
      if (filter.updatedTo)   (where.updatedAt as any).lte = new Date(filter.updatedTo + 'T23:59:59');
    }
