...
export class OrderRepository {
  async findAll(filter: OrderFilterQuery & { includeDeleted?: boolean }) {
    ...
    // Phase 6.5 §6 — Order source and test-order filter
    if ((filter as any).orderSource) (where as any).orderSource = (filter as any).orderSource;
    if ((filter as any).isTest !== undefined) (where as any).isTest = (filter as any).isTest === 'true' || (filter as any).isTest === true;
